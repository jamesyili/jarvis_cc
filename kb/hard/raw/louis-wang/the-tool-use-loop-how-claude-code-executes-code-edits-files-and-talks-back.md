...
**Source:** https://louiswang524.github.io/blog/claude-code-tool-use-loop/
**Ingested:** 2026-04-05
**Tags:** llms, recsys, ai-agents, ml-systems, claude-code

---

[Home](/) › [Blog](/blog) › The Tool Use Loop: How Claude Code Executes Code, Edits Files, and Talks Back  
The Tool Use Loop: How Claude Code Executes Code, Edits Files, and Talks Back
 
 March 31, 2026 · 8 min read  · [Claude Code](/tags/Claude Code)[tool use](/tags/tool use)[LLM](/tags/LLM)[AI agents](/tags/AI agents)[Anthropic API](/tags/Anthropic API) 
...
[The Query-Stream-Tool Loop](#the-query-stream-tool-loop)
...
[Parallel vs. Serial Execution](#parallel-vs-serial-execution)
...
Text appears in real time as the model explains what it will do.

A pause occurs while tools execute (the agent runs the requested operations).

More text appears as the model processes the results and continues.
...
[Design Lesson](#design-lesson)

**Design Lesson: Tools are your agent’s API surface**

**Schema design is load-bearing.** The LLM selects tools based on description and input schema. Write tool descriptions like API documentation, not internal comments. Wrong description = tool never called, or called when it shouldn’t be.

**Tools should do one thing.** Claude Code has 40+ tools, each narrowly scoped: `BashTool` runs shell commands, `FileEditTool` makes targeted edits, `FileReadTool` reads files. A tool that does three things forces the LLM to reason about three things at once.

**Denials are data.** When a tool call is denied, Claude Code returns a structured `tool_result` with `is_error: true` and a denial message from the `REJECT_MESSAGE` constant, explaining the operation was denied. The LLM can adapt — try a different approach, ask the user, or stop. Silently swallowing a denial leaves the LLM confused about why its plan didn’t work.
...
**`hooks/toolPermission/PermissionContext.ts`** — Permission gate context and deny/allow decision flow
 ...
 [ ← Previous Demystifying Claude Code: Inside the Architecture of a CLI Code Agent ](/blog/demystifying-claude-code-architecture) 
 
 [ Next → Security Without a Sandbox: How Claude Code Decides What It's Allowed to Do ](/blog/claude-code-security-permissions)
