...
[Home](/) › [Blog](/blog) › Building a Self-Improving Personal Knowledge Base Powered by LLM  
Building a Self-Improving Personal Knowledge Base Powered by LLM
 
 April 5, 2026 · 15 min read  · [LLM](/tags/LLM)[knowledge management](/tags/knowledge management)[Claude Code](/tags/Claude Code)[AI agents](/tags/AI agents)[personal tools](/tags/personal tools) 
...
[The Problem With How We Manage Knowledge Today](#the-problem-with-how-we-manage-knowledge-today)
...
**3. Outputs** — Q&A answers, synthesis reports, lint reports, slides, charts. These get filed back into the wiki so future queries can reference them.
 ...
 LLM Knowledge Base — architecture flow   
       ...
       Your Content   URLs   PDFs   Images   Notes  
 ...
 synthesis     wiki/  —  LLM-managed index.md   ·   concepts/   ·   sources/   ·   archive/ master index   ·   concept articles   ·   source summaries   ·   merged articles     
   ...
   Q&A answers outputs/*.md   health reports outputs/lint-*.md   merged articles wiki/archive/   slides & charts outputs/*.md / *.png    
...
[`/kb-ingest` — Staged Intake](#kb-ingest--staged-intake)
...
[`/kb-compile` — The Wiki Compiler](#kb-compile--the-wiki-compiler)

This is the core skill. For each uncompiled raw file, the LLM:

Reads the content

Writes `wiki/sources/<slug>.md` — a structured summary with tags, key concepts, and notable details

Extracts concepts and creates or updates `wiki/concepts/<concept>.md`, adding `[[backlinks]]` to the source

Appends one-line entries to `wiki/index.md`

Updates the manifest to `status: compiled`
...
[`/kb-ask` — Index-First Q&A](#kb-ask--index-first-qa)
...
[`/kb-reflect` — The Self-Improvement Engine](#kb-reflect--the-self-improvement-engine)
...
[`/kb-lint` — Health Checks](#kb-lint--health-checks)
...
**Thin articles** — concept articles with fewer than 3 substantive sentences

**Missing concepts** — `[[concepts/X]]` links in sources with no corresponding article

**Broken wikilinks** — links pointing to files that don’t exist

**Duplicate concepts** — near-duplicate slugs (`attention` and `attention-mechanism`)

**New article suggestions** — gaps in the wiki, with optional web search to impute missing data

Output: terminal summary + a full report in `outputs/` filed back into the wiki.

[`/kb-merge` — Concept Consolidation](#kb-merge--concept-consolidation)
...
[`/kb-output` — Rendering](#kb-output--rendering)
...
[`/kb-import` — Smart Obsidian Vault Import](#kb-import--smart-obsidian-vault-import)
...
[`/kb-merge-vault` — Combining Two KB Vaults](#kb-merge-vault--combining-two-kb-vaults)
...
**Keyword search** — TF-IDF scoring with 3x title boost, fast, no dependencies

**Semantic fallback** — if keyword confidence is below threshold, encode the query with `sentence-transformers` (all-MiniLM-L6-v2) and cosine-similarity against cached embeddings
...
That’s the part that feels qualitatively different from search. Search finds what you know is there. This surfaces what you didn’t know you knew.
 ...
 [ ← Previous Gemma 4 Explained: How One Model Family Spans Phones and Frontier-Class Reasoning ](/blog/gemma-family)
