...
**Source:** https://louiswang524.github.io/blog/claude-code-security-permissions/
**Ingested:** 2026-04-05
**Tags:** llms, recsys, ai-agents, ml-systems, claude-code

---

[Home](/) › [Blog](/blog) › Security Without a Sandbox: How Claude Code Decides What It's Allowed to Do  
Security Without a Sandbox: How Claude Code Decides What It's Allowed to Do
 
 March 31, 2026 · 11 min read  · [Claude Code](/tags/Claude Code)[security](/tags/security)[permissions](/tags/permissions)[AI agents](/tags/AI agents)[tool use](/tags/tool use) 
...
[The Permission Gate](#the-permission-gate)
...
[Bypass Mode (and the Kill Switch)](#bypass-mode-and-the-kill-switch)
...
[The Audit Surface](#the-audit-surface)
...
[What Claude Code Doesn’t Do](#what-claude-code-doesnt-do)
...
**Put the gate at the tool boundary, not inside tools.** `hasPermissionsToUseTool()` wraps every tool call from outside. Tools don’t self-authorize. Change the permission model once; all tools inherit it.

**Classify inputs, not just tools.** Approving “BashTool” is meaningless — `rm -rf /` and `echo hello` are both BashTool calls. The permission model classifies the *input* through dangerous patterns, tool-specific `checkPermissions()`, and an AI classifier. Each layer catches different risks.

**Design for the trusted-user case first.** A kernel sandbox is the right answer for hostile inputs. For a tool running on a developer’s own machine, explicit permission layers with dangerous-pattern detection is the pragmatic trade-off. Ship the useful thing, then harden.
...
**`filesystem.ts`** — path safety checks for `.git/`, `.claude/`, `.vscode/`, and shell configs
 ...
 [ ← Previous The Tool Use Loop: How Claude Code Executes Code, Edits Files, and Talks Back ](/blog/claude-code-tool-use-loop) 
 
 [ Next → Designing for Extensibility: How Claude Code's Plugin and Skill System Works ](/blog/claude-code-plugin-skill-system)
