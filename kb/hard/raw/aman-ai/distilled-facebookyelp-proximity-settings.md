...
* [High Level System Design](#high-level-system-design)
  + [SQL solution](#sql-solution)
    - [How efficient would this query be?](#how-efficient-would-this-query-be)
  + [Grids](#grids)
    - [What could be a reasonable grid size?](#what-could-be-a-reasonable-grid-size)
    - [Should we keep our index in memory?](#should-we-keep-our-index-in-memory)
  + [Dynamic size grids](#dynamic-size-grids)
    - [What data-structure can hold this information?](#what-data-structure-can-hold-this-information)
    - [How will we build a QuadTree?](#how-will-we-build-a-quadtree)
    - [How will we find the grid for a given location?](#how-will-we-find-the-grid-for-a-given-location)
    - [How will we find neighboring grids of a given grid?](#how-will-we-find-neighboring-grids-of-a-given-grid)
    - [What will be the search workflow?](#what-will-be-the-search-workflow)
    - [How much memory will be needed to store the QuadTree?](#how-much-memory-will-be-needed-to-store-the-quadtree)
    - [How would we insert a new Place into our system?](#how-would-we-insert-a-new-place-into-our-system)
* [Detailed Component Design](#detailed-component-design)
* [Data Partitioning](#data-partitioning)
  + [Sharding based on regions](#sharding-based-on-regions)
    - [What if a region becomes hot?](#what-if-a-region-becomes-hot)
  + [Sharding based on `LocationID`](#sharding-based-on-locationid)
    - [Will we have different QuadTree structure on different partitions?](#will-we-have-different-quadtree-structure-on-different-partitions)
* [Replication and Fault Tolerance](#replication-and-fault-tolerance)
  ...
  + [How can we efficiently retrieve a mapping between Places and QuadTree server?](#how-can-we-efficiently-retrieve-a-mapping-between-places-and-quadtree-server)
* [Cache](#cache)
* [Load Balancing (LB)](#load-balancing-lb)
* [Ranking](#ranking)
  + [How can we return most popular places within a given radius?](#how-can-we-return-most-popular-places-within-a-given-radius)
...
* Non-functional Requirements:
  + **Minimum latency**: Users should have a real-time search experience with minimum latency.
  + **High availability:** The final design should be highly available.
  + **Eventual consistency (due to [CAP theorem](../cap-theorem)):** As we know from the CAP theorem, that we can have either high availability or high consistency, thus we will aim for an eventually consistent system. Consistency can take a hit (in the interest of availability) if a location/event doesn’t show up for a little while; it should be fine.
  + **High reliability**: The system should be highly reliable; any added location/event should never be lost.
  + **Read-heavy service:** Our service should support a heavy search load. There will be a lot of search requests compared to adding a new place.
* Extended Features:
  + Query events happening near a given place around a particular time. This basically adds the third dimension of time to the problem.
...
* **Number of places**: 500M
* **Queries per second**: 5B/86400 = ~60K QPS
* **Year-on-Year (YoY) growth in number of places and QPS**: 20%

## System APIs

* We can have SOAP or REST APIs to expose the functionality of our service.
* The following could be the definition of a read API to fetch the nearby places for each user query around a given location:

  ```
    searchNearbyPlaces(api_dev_key, search_terms, user_location, radius_filter, maximum_results_to_return, category_filter, sort, page_token)
  ```
  ...
  + Returns (JSON): A JSON containing information about a list of businesses matching the search query. Each result entry will have the business name, address, category, rating, and thumbnail.
* The following could be the definition of a write API to add a new place and store it in our database:

```
addPlace(api_dev_key, place_name, place_location, timestamp)
```

* Parameters:
  + `api_dev_key (string):` The API developer key of a registered account. This will be used to, among other things, throttle users based on their allocated quota.
  + `place_name (string):` A string containing the place name.
  + `place_location (string):` Location of the place.
  + `timestamp (number):` The current timestamp to send places with timestamps associated greater than the current time – can also optionally send the ID (if the ID is encoded based on the timestamp) of the latest comment to fetch places greater than this ID).
* Returns (JSON): A JSON containing information about a list of businesses matching the search query. Each result entry will have the business name, address, category, rating, and thumbnail.
...
* One simple solution could be to store all the data in a database like MySQL. Each place will be stored in a separate row, uniquely identified by `LocationID`. Each place will have its longitude and latitude stored separately in two different columns, and to perform a fast search; we should have **indexes on both these fields (longitude and latitude)**.
* To find the `LocationIDs` of all the nearby places, given a location `(X, Y)` within a radius `D`, we can query like this:
...
* We can divide the whole map into smaller grids to group locations into smaller sets. Each **grid will store all the places residing within a specific range of longitude and latitude**. This scheme would enable us to query only a few grids to find nearby places.
* We maintain a hashmap of `(lat, long)` endpoints to a `GridID`. So if you’re within `(lat_start, long_start)` and `(lat_end, long_end)`, you’d be assigned a particular `GridID` to look for (and it’s neighboring IDs based on the search radius `D`). Thus, based on a **given location and radius**, we can find all the neighboring grids and then query these grids to find nearby places.
...
* Maintaining the index in memory will improve the performance of our service (using an in-memory data structure store, say [Redis](https://en.wikipedia.org/wiki/Redis)). We can keep our index in a hash table where ‘key’ is the **grid number** and ‘value’ is the **list of places** contained in that grid.
...
* Issue with **fixed size grids**:
  ...
  + This problem can be solved if we can dynamically adjust our grid size such that **whenever we have a grid with a lot of places we break it down to create smaller grids**. A couple of challenges with this approach could be:
    - How to map these grids to locations, and
...
* A tree in which each node has four children, called a [QuadTree](https://en.wikipedia.org/wiki/Quadtree), can serve our purpose. Each node will represent a grid and will contain information about all the places in that grid. If a node reaches our limit of 500 places, we will break it down to create four child nodes under it and distribute places among them. In this way, all the leaf nodes will represent the grids that cannot be further broken down. So leaf nodes will keep a list of places with them.

* In summary,
  + Use a data structure called QuadTree to represent the dynamic size grids.
  + A QuadTree is a tree where each node has four children.
...
* In summary,
  + Denote each **grid as a node** in the QuadTree.
  + If a particular grid has more places than a **specific threshold**, we will divide that grid into **four more grids**, meaning that the node will have **four children containing places from each quadrant**, as shown in the above image.

#### How will we find the grid for a given location?

* We will start with the root node and search downward to find our required node/grid. At each step, we will see if the current node we are visiting has children. If it has, we will move to the child node that contains our desired location and repeat this process.
* If the node does not have any children, then that is our desired node.

#### How will we find neighboring grids of a given grid?

* Since only **leaf nodes contain a list of locations**, we can connect all leaf nodes with a doubly linked list. This way we can **iterate forward or backward** among the neighboring leaf nodes to find out our desired locations.
* Another approach for finding adjacent grids would be through **parent nodes**. We can keep a **pointer in each node to access its parent**, and since each parent node has pointers to all of its children, we can easily find siblings of a node. We can keep expanding our search for neighboring grids by going up through the parent pointers.
...
* For each Place, if we cache only `LocationID` and Lat/Long, we would need 12GB to store all places: 24 \* 500M => 12 GB.
* Since each grid can have a maximum of 500 places, and we have 500M locations, how many total grids we will have? 500M / 500 => 1M grids.
* Which means we will have 1M leaf nodes and they will be holding 12GB of location data. A QuadTree with 1M leaf nodes will have approximately 1/3rd internal nodes, and each internal node will have 4 pointers (for its children). If each pointer is 8 bytes, then the memory we need to store all internal nodes would be: 1M \* 1/3 \* 4 \* 8 = 10 MB.
...
* There would be a lot of queries on the server holding that region, making it perform slow. This will affect the performance of our service.
...
* To recover from these situations, either we have to repartition our data or use [consistent hashing](../consistent-hasing).
...
* We can add LB layer at two places in our system:
  + Between Clients and Application servers and,
...
* [Designing Uber backend](../uber), discusses dynamic updates of the QuadTree in detail.
