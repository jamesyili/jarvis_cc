...
**Source:** https://eugeneyan.com//writing/challenges-after-deploying-machine-learning/
**Ingested:** 2026-04-02
...
I’ll discuss this from the bottom-up: starting with [data](#1-schemas-change-when-they-shouldnt--vice-versa), then [models](#2-unwanted-model-interactions-internal--external) and [engineering](#3-infra--codebase-is-messy-read-chaotic), then aspects external to the system (i.e., [the real world](#4-real-world-bias-and-bad-guys-come-knocking), [org structure](#5-org-issues---priorities-clash-diffused-responsibility), [users](#6-with-users-you-have-to-provide-customer-service)).

1. [Schemas Change When They Shouldn’t (& vice versa)](#1-schemas-change-when-they-shouldnt--vice-versa)
2. [Unwanted Model Interactions (Internal & External)](#2-unwanted-model-interactions-internal--external)
3. [Infra & Codebase is Messy (read: Chaotic)](#3-infra--codebase-is-messy-read-chaotic)
4. [Real-World Bias and Bad Guys Come Knocking](#4-real-world-bias-and-bad-guys-come-knocking)
5. [Org Issues - Priorities Clash; Diffused Responsibility](#5-org-issues---priorities-clash-diffused-responsibility)
6. [With Users, You Have to Provide Customer Service](#6-with-users-you-have-to-provide-customer-service)

Feel free to skip ahead. In the *next* spost, we’ll discuss how to tackle these challenges.

> Follow-up post: [A Practical Guide to Maintaining Machine Learning](/writing/practical-guide-to-maintaining-machine-learning/)
...
Suppose you’re building a model to  [estimate a patient’s hospital bill](/speaking/machine-learning-in-production-for-healthcare-talk/). Patients request an estimation at the point of admission (or when they're shopping around). A key feature in your model is the *estimated* `hospitalization_days` (provided by the admitting doctor); this is stored in the production database. One day, someone decides to update the `hospitalization_days` column with the actual number of days hospitalised.
...
**Thanks** to Yang Xinyi, [Stew Fortier](https://stewfortier.com/), [Richie Bonilla](http://richie.co/), and [Joel Christiansen](https://joelchristiansen.com) for reading drafts of this.

  

If you found this useful, please cite this write-up as:

> Yan, Ziyou. (May 2020). 6 Little-Known Challenges After Deploying Machine Learning. eugeneyan.com.
