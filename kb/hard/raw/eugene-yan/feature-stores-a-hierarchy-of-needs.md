...
**Source:** https://eugeneyan.com//writing/feature-stores/
**Ingested:** 2026-04-02
...
* [Feature Store Hierarchy of Needs](#the-feature-store-hierarchy-of-needs)
* [Access: Reducing duplication & encouraging reusability](#access-reducing-duplication--encouraging-reusability)
* [Serving: Using features in real-time environments](#serving-using-features-in-real-time-environments)
* [Integrity: Creating correct offline and online features](#integrity-creating-correct-offline-and-online-features)
* [Convenience: Making it as easy to use as possible](#convenience-making-it-as-easy-to-use-as-possible)
* [Autopilot: Automating as much as we can](#autopilot-automating-as-much-as-we-can)

> This post is now [available in Mandarin](https://yiksanchan.com/posts/feature-stores-a-hierarchy-of-needs) thanks to [Yik San](https://twitter.com/yiksanchan)!
...
Maslow's hierarchy of needs as a pyramid, with more basic needs at the base. ([source](https://en.wikipedia.org/wiki/Maslow's_hierarchy_of_needs))
...
Borrowing from The Innovator's Dilemma

Parts of the feature store hierarchy of needs are borrowed from the product evolution model in [The Innovator’s Dilemma](https://en.wikipedia.org/wiki/The_Innovator%27s_Dilemma). It is also called the buying hierarchy by its creators, Windermere Associates of San Francisco, California.
...
Creating batch (left) and real-time (right) features, and synchronizing between stores ([source](https://www.infoq.com/presentations/michelangelo-palette-uber/))
...
Creating snapshots from offline stores and online micro services ([source](https://netflixtechblog.com/distributed-time-travel-for-feature-generation-389cccdd3907))
...
GoJek's Feast ingesting features via Apache Beam ([source](https://www.gojek.io/blog/feast-bridging-ml-models-and-data))
...
Netflix uses the same encoders for offline and online feature generation ([source](https://databricks.com/session/fact-store-scale-for-netflix-recommendations))
...
Data quality scores over time, and when incidents occur ([source](https://eng.uber.com/monitoring-data-quality-at-scale/))
...
Uber Palette's domain-specific language (code examples)

Uber also provided a [detailed example](https://www.infoq.com/presentations/michelangelo-palette-uber/) of how they extended the [Spark Transformer](https://spark.apache.org/docs/latest/ml-pipeline.html#transformers) (no, that that NLP transformer) and built a domain-specific language (DSL) for feature retrieval and transformation. This is done via two abstractions:

* Transformer: Despite the name, these are mainly used for feature retrieval.
* Estimator: Not for training models like [Spark’s Estimator](https://spark.apache.org/docs/latest/ml-pipeline.html#estimators), but mainly used to create features.
...
Airbnb's UI for backfilling and the Airflow DAG it creates ([source](https://speakerdeck.com/artwr/using-apache-airflow-as-a-platform-for-data-engineering-frameworks?slide=17))
...
>
> — Eugene Yan (@eugeneyan) [February 24, 2021](https://twitter.com/eugeneyan/status/1364384045975539712?ref_src=twsrc%5Etfw)
...
**Thanks** to Yang Xinyi, [Willem Pienaar](https://twitter.com/willpienaar), [David Golden](https://xdg.me/), [Shawn Wang](https://twitter.com/swyx), and [Sky Chin](https://twitter.com/co0lsky) for reading drafts of this.

  

If you found this useful, please cite this write-up as:

> Yan, Ziyou. (Feb 2021). Feature Stores: A Hierarchy of Needs. eugeneyan.com.
> https://eugeneyan.com/writing/feature-stores/.

or

```
@article{yan2021feature,
  title   = {Feature Stores: A Hierarchy of Needs},
  author  = {Yan, Ziyou},
  journal = {eugeneyan.com},
  year    = {2021},
  month   = {Feb},
  url     = {https://eugeneyan.com/writing/feature-stores/}
}
```
