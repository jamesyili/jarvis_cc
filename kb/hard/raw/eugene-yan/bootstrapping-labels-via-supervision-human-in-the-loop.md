...
**Source:** https://eugeneyan.com//writing/bootstrapping-data-labels/
**Ingested:** 2026-04-02
...
Another approach is [information density](https://dl.acm.org/doi/10.5555/1613715.1613855), where points that are most uncertain *and* representative of the data distribution are selected. Assume we have two samples that have similar uncertainty. One has many samples close to it, while the other is an outlier. In this case, we would pick the sample that has other samples close to it.

While point A has highest uncertainty, B has a mix of uncertainty and information density ([source](https://www.youtube.com/watch?v=6q7c6Dlw0JI))
...
Doordash's active learning workflow, with automated steps in green and HITL steps in red ([source](https://doordash.engineering/2020/08/28/overcome-the-cold-start-problem-in-menu-item-tagging/))
...
Nearest neighbors allowed similar performance (as active learning) with 2 - 15% of data ([source](https://arxiv.org/abs/2007.00077))
...
Template labeling function written in C++ ([source](https://arxiv.org/abs/1812.00417))
...
An overview of Snorkel Drybell outline in dashed blue lines ([source](https://arxiv.org/abs/1812.00417))
...
>
> — Andrej Karpathy (@karpathy) [July 8, 2021](https://twitter.com/karpathy/status/1413242233394929664?ref_src=twsrc%5Etfw)
...
Alternatively, we can start with approaches that don’t require user data. How do we build semantic search without user interaction labels? One way is to start with [lexical approaches](/writing/search-query-matching/#lexical-based-the-bedrock-of-query-processing) such as [BM25](https://en.wikipedia.org/wiki/Okapi_BM25) or whatever’s available in ElasticSearch or Solr. Once deployed, we can then collect user interaction data (e.g., clicks, purchases) on search results, which can then be used as labels for semantic search models.
...
Did I miss any key techniques for bootstrapping labels? Please [reach out](https://twitter.com/eugeneyan) and let me know!
...
>
> — Eugene Yan (@eugeneyan) [August 4, 2021](https://twitter.com/eugeneyan/status/1422713392914059265?ref_src=twsrc%5Etfw)

Thanks to [Lim Wen Qing](https://www.linkedin.com/in/wen-qing-lim-67330771/) and [Nihit Desai](https://twitter.com/nihit_desai) for pointing me to great resources. Also thanks to Nihit Desai and [Samuel Flender](https://www.linkedin.com/in/sflender/) for providing feedback.

  

If you found this useful, please cite this write-up as:

> Yan, Ziyou. (Aug 2021). Bootstrapping Labels via \_\_\_ Supervision & Human-In-The-Loop. eugeneyan.com.
> https://eugeneyan.com/writing/bootstrapping-data-labels/.

or

```
@article{yan2021labeling,
