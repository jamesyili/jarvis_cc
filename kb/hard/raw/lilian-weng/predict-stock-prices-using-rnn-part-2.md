...
Date: July 22, 2017 | Estimated Reading Time: 9 min | Author: Lilian Weng
...
In the recurrent neural network, at one time step $t$, the input vector contains `input_size` (labelled as $w$) daily price values of $i$-th stock, $(p\_{i, tw}, p\_{i, tw+1}, \dots, p\_{i, (t+1)w-1})$. The stock symbol is uniquely mapped to a vector of length `embedding_size` (labelled as $k$), $(e\_{i,0}, e\_{i,1}, \dots, e\_{i,k})$. As illustrated in Fig. 1., the price vector is concatenated with the embedding vector and then fed into the LSTM cell.
