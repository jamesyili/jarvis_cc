...
\log \mathcal{L}(\theta)
&= \log p(y \mid x) \\
&= \log \sum\_{k=1}^K p(y, z^{(k)} \mid x) \\
&= \log \sum\_{k=1}^K p(z^{(k)} \mid x)\; p(y \mid z^{(k)}, x) \\
...
\begin{aligned}
\nabla\_\theta J(\theta)
&= \nabla\_\theta \mathbb{E}\_{z\_i, y\_i \sim p(.\mid x\_i)} \mathbb{1}[y\_i = y\_i^\text{truth}] \\
&= \sum\_{i=1}^N \nabla\_\theta \mathbb{1}[y\_i = y\_i^\text{truth}] \; p(y\_i, z\_i \mid x\_i) \\
&= \sum\_{i=1}^N \mathbb{1}[y\_i = y\_i^\text{truth}] \; p(y\_i, z\_i \mid x\_i) \frac{\nabla\_\theta p(y\_i, z\_i \mid x\_i)}{p(y\_i, z\_i \mid x\_i)} & \text{;log-derivative trick}\\
&= \mathbb{E}\_{z\_i, y\_i \sim p(.\mid x\_i)} \mathbb{1}[y\_i = y\_i^\text{truth}] \; \nabla\_\theta \log p(y\_i, z\_i \mid x\_i) & \text{;log-derivative trick}
\end{aligned}
$$
