...
*Pinkerton + Reflex — Q2 2026 Vision Memo*
...
- **Pinkerton M0 shipped 2026-04-07 week** (two PRs; full-funnel logging live).
...
### Why this compounds
**Pinkerton is the observatory; Reflex is the astronomer.** Pinkerton renders the recsys signal landscape legible (14-stage request forensics, user profiles, scale analysis). Reflex forms hypotheses and generates investigation work. They are complementary, not merged — Pinkerton is a platform that can serve multiple consumers; Reflex is the first agent on top. This separation is intentional: it keeps Pinkerton available as infrastructure for future agents, cross-surface extensions, and human-initiated debugging.
...
`Signals → Pinkerton (investigation engine) → Reflex (autonomous agent) → Hypothesis Cards → Engineer investigates → RLHF feedback → Reflex improves`

Six-stage Reflex pipeline (Andrew's vision): Detect → Diagnose → Design → Verify → Experiment → Explain. Pinkerton M3 + PINvestigator = Detect. Pinkerton M1 + M2 + Reflex reasoning = Diagnose. Pinkerton Phase 4 simulation (post-validation gate) = Verify.

### Q2 milestones
| Milestone | Timing | Owner |
|---|---|---|
| Pinkerton M1 (Request Debugger) in prod | End April | Chuxi + James |
| Reflex co-dev kickoff | 2026-04-14 | Andrew + James |
| Pinkerton M2 (User Understanding) → Reflex feed | May | James's team |
| First 10 RLHF-validated Reflex cards | End May | James + Andrew |
| M3 scale run on 1 HF segment | June | James's team |

### Team
Alok (PhP lead, Pinkerton tech lead), Chuxi (20% committed), Daniel (logging), James (investigation engine + Reflex co-dev + RLHF expert). Sponsor chain: Dylan (engineering), Andrew (product). **[FILL: Anna's last name]** (PM, Retentive Recs) bridges the 4-way nexus.

## What Leadership Can Do

1. **Hold the architectural frame.** When Pinkerton/Reflex surfaces in leadership forums, the line is: *Pinterest is building the autonomous quality monitoring platform for recommendations; Pinkerton is the substrate, Reflex is the agent, James + Andrew are co-owners.* That sentence does the work.
