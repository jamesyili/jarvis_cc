...
This file holds **Claude Code-specific extensions** on top of that base: the slash-command skill registry, custom sub-agents, `settings.local.json` hooks, and the file-based memory system. The skills and agent specifications are canonical here; Codex discovers ordinary entry-point files in `.agents/skills/` that read `.claude/skills/`, and loads compatible agents, hooks, and the NotebookLM connection from `.codex/`. Install user-level skill entries with `scripts/leo_setup.py --user`. Windows/Linux setup and root detection: `system/leo-portability.md`. Gemini, Cursor, and Aider work from `AGENTS.md` and the flattened workflows in `prompts/`.
...
|-------|-------|------|---------|---------|
| **Consult-Notebook** | Sonnet | Background | Keyword triggers (see below) or proactive consultation | Queries NotebookLM notebooks in isolation — keeps verbose NLM results out of main context. Appends raw response to `system/notebooklm/query_log.md` as audit trail. **Local machines only** — the NotebookLM MCP is not bound in remote/cloud sessions; the agent fails loudly with no query sent (verified 2026-08-15). Don't spawn it remotely; note the limitation and offer to run it next local session. |
