...
> **Source:** https://aman.ai/primers/ai/preference-optimization/ — offline text copy of the aman.ai primer (companion to `interview_prep/aman_rl_ppo.pdf`, which is image-only and not text-extractable).
...
![](/primers/ai/assets/DeepSeek/PPO-GRPO.jpg)
...
* PPO’s clipped surrogate objective function has the following components:
  ...
  + Formally:
    ...
    \min\left(
    \underbrace{r\_t(\pi\_\theta) \hat{A}^{\text{GAE}}\_t}\_{\text{unclipped objective}},\,
    \underbrace{\text{clip}(r\_t(\pi\_\theta),1-\epsilon, 1+\epsilon) \hat{A}^{\text{GAE}}\_t}\_{\text{clipped objective}}
    \right)
    \right]\]
    - where:
      * \(L^{\text{clip}}(\theta)\):
...
* **Reference Model (\(\pi\_{\text{ref}}(y \mid x)\)):**
  ...
  + A frozen copy of the SFT model that provides a **stable distributional baseline**.
...
![](/primers/ai/assets/rlhf/openAI.png)
...
![](/primers/ai/assets/rlhf/chip.jpg)
...
* **What distributions is the KL divergence between?**
  ...
  + In RLHF, PPO is reused as the optimization mechanism, but the **KL divergence is redefined**. Instead of comparing the current policy to the immediately previous policy, RLHF defines the KL divergence between:
    ...
    - where:
      ...
      * \(\pi\_{\text{ref}}\) is a **fixed reference policy**, typically the **SFT model**.
  + Key properties of these distributions:
...
* The reference model is architecturally **identical** to the policy model. It is often just a **frozen copy** of the SFT model.
* Example pipeline:
...
![](/primers/ai/assets/rlhf/llama.jpeg)
  ...
  4. **KL Regularization**:
     ...
     + GRPO introduces a **per-token KL divergence penalty** to stabilize optimization and constrain the updated policy’s drift from the reference:
       ...
       - where:
         ...
         * \(\pi\_{\text{ref}}(o\_{i,t} \mid q, o\_{i,<t})\) is the **probability under the frozen reference policy**, often the SFT or pre-trained model.
...
![](/primers/ai/assets/DeepSeek/DAPO_entropy.jpg)
...
![](/primers/ai/assets/DeepSeek/DAPO.jpg)
...
![](/primers/ai/assets/DeepSeek/GRPOplus.webp)
...
![](/primers/ai/assets/preference-optimization/GSPO_fig1.jpg)
...
![](/primers/ai/assets/preference-optimization/CISPO_fig2.jpg)
...
![](../../../images/papers/RewardTampering.jpg)
...
* **Implementation Details:**
  ...
  3. **Training Setups**:
     + **Base Setup**: Models were trained on the UltraChat-200k dataset to create a SFT model, followed by preference optimization using the UltraFeedback dataset.
...
* **Model Development and Implementation**:
  ...
  2. **Data Selection and Processing:**
     ...
     ![](../../../images/papers/DeepSeekMath-1.jpg)
  ...
  4. **RL with GRPO:**
     ...
     ![](../../../images/papers/DeepSeekMath-2.jpg)
...
* **Architecture and Implementation**:
  ...
  ![](../../../images/papers/Understanding-R1-Zero-Like-Training.jpg)
* **Core Insights**:
