...
## Addendum 3 — 2026-08-27: the TPM arrived, the workstream widened, metrics support offered (Michael, new paragraph appended to the same DM)

**Michael, verbatim (pasted by James 8/27 AM; the earlier body of the message is the 8/21 text above, already answered 8/25):**
> A few updates on my end.
> We now have a TPM to help support the broader 'Safe Journeys' workstream which now includes anything that intersects with borderline content distribution (🤦 ) - Ambica Bhardwaj. We're working on coming up with a set of overarching milestones we can start working backward from. Ambica is going to reach out to Melissa Saboowala about metrics support overall for Safe Journeys. This is the workstream in which you need support, is that correct? Additionally, I'll be trying to stand up a 'Safe Journeys' success metrics proposal, which I'll share with you to see if there's anything we should add / remove.

**What it changes (Leo read, 8/27):**
1. **TPM reversal.** 8/25 4:35 PM: "It seems like we're NOT going to get TPM support 😔" → now **Ambica Bhardwaj** (unknown to the repo — role/chain to fill). Tracking leaves Michael + James's hands; the milestones-working-backward exercise is hers. James's input there = the doc's phased timeline (Tab 1) — hand it over as the eng-side spine so the TPM's milestones don't get invented fresh.
2. **Scope creep, named by its owner.** "Anything that intersects with borderline content distribution" — Michael's own 🤦. The framework's scope-outs (WS1 metric productionization; WS5 wellbeing UX; user-level seeker work) are now the thing to defend, or "Safe Journeys" becomes the org's bucket for every quality complaint. **Watch line:** the framework doc stays scoped to WS2/WS3; the *program* can be wide.
3. **Metrics support — the question to answer precisely.** "This is the workstream in which you need support, is that correct?" The doc's §9 #3: measurement DS is **staffed for GenAI (Stephanie Chen UXR, Wenjun Wang DS) and unstaffed for self-harm.** So: yes — specifically **WS1 for the self-harm signal** (the USR metric definition + productionization the framework scoped out as someone else's) **and the retention holdout** (§9 #4). Not a general "we need DS." **Melissa Saboowala** — unknown to the repo; presumably DS/analytics (Manu Sharma's org or PADS/Karim §42 — verify before assuming).
4. **Success-metrics proposal incoming.** James's add/remove list already exists in the framework: the regrettable/non-regrettable decomposition (D5), the SSv2 guardrail budget, hold-back → derived λ, and the false-positive asymmetry for visible interventions (placement doctrine). Those are the four things a PM-authored metrics proposal is most likely to omit.

**Open:** who Ambica Bhardwaj and Melissa Saboowala are; whether the 8/28 plan share to Andrew/Dylan/Faisal is still on for tomorrow.
