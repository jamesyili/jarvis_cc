# P13N Retrieval and Anticipation — Org Design (Version B: executive)

**July 24, 2026 · James Li · Draft for Dylan** *(revises the 2026-07-19 exec-voice draft: adds a BLUF summary, restores "Calls I've made," first-person authorship pass; late-7/24 session folded in the locked org + team names and the portfolio-balance calls)*
...
*Pre-send flags (strip before pasting): (1) **Org name LOCKED 7/24 as "P13N Retrieval and Anticipation"** — Dylan's approved 7/20 string minus the redundant "ML," parallel to Dhruvil's "P13N Ranking and Blending." Still needs: 30-second Dylan ratification of the trim, and reconciliation into the announcement email (currently "Personalization Retrieval and Anticipation ML") + talking points before anything ships. Dhruvil's Rahul/anticipation concern was raised and deflated in the 7/24 Slack thread ("got it"). (2) A sibling version (`org_design_doc_2026-07-19_direct_voice.md`) exists — reconcile and pick one before pasting into Dylan's GDoc. (3) Team names below (Retrieval Foundations / Retrieval Modeling / Anticipation Modeling) are LOCKED 7/24; the workstream tables still show pre-naming allocations in places — reconcile GenRet's target home (Alim) before sending.*

## Summary

**Proposal.** Consolidate the full anticipation stack — from user signal to what the ranker sees — into one organization by merging Curation ML into P13N CG. Structure: three teams — **Anticipation Modeling** (Alim), **Retrieval Modeling** (Daniel), and **Retrieval Foundations** (my lean, individually-justified direct pod: UPP, CLR transitionally, Reflex, Foundations & Efficiency). The two "Retrieval" teams split cleanly: Foundations owns the framework and substrate the org builds on; Modeling owns the models that run on it. My internal reorganization is decoupled from your org-chart move and runs on its own clock, using a deliberate observation window before the larger internal moves.

**Calls I've already made** (detail in *Proposal → Calls I've made*). LWS moves to Daniel on day one; CLR stays with me transitionally and targets Alim at the settle point; UPP stays with me as the shared framework, anchored by Piyush; Alim's initial pod is locked (Chuxi, Yidi, Alok, Lionel); JJ splits Reflex and Foundations & Efficiency, with Rui as the L1/Real-Time operational owner.

**Decisions I need from you.**
1. **Balaji placement** — platform TL under Daniel, or Staff anchor under Alim. This intersects what Alim heard during closing, so your read matters.
2. **Kim's loan to Dhruvil** — I'd like it wound down so her time returns to this organization; cleanest if you set it up with Dhruvil directly, with the reorg as natural cover.

*The strategic case, charter, personnel design, and full proposal follow.*
...
- Establishes pods that own a Pinner problem end to end, with the decision rights to act on it without cross organizational permission.
...
The organization runs three teams, each carrying the same topline goals: **SSv2, WAU, and Cost Savings.** Each team pairs a workstream that is reliably producing gains with frontier bets — no team is all-risk, no team is all-maintenance.

| Team | Scope |
|---|---|
| **Anticipation Modeling** (Alim) | pUIC (model based and LLM based), Retentive Recs, Unified Explore Backend / Explore Page; Generative Retrieval joins at the settle point as the team's proven gains engine |
| **Retrieval Modeling** (Daniel) | LWS (lightweight preranking — the reliable gains engine), Intelligent Boards (the frontier bet, placement gated below), Recommended Boards |
| **Retrieval Foundations** (James) | UPP, the shared retrieval framework the modeling charters build on; CLR transitionally; Reflex and agentic dev velocity systems; Foundations & Efficiency (responsiveness, L1 utility, cost) |

CLR converges into one of the first two teams at the settle point, sized by where headcount lands under this design.
...
- **Unified modeling investment across boards and retrieval.** Board recommendation modeling and retrieval modeling are adjacent disciplines. Locating boards modeling and LWS preranking modeling in the Retrieval Modeling subcharter lets the techniques and infrastructure transfer directly.

## Additional Design Considerations

Beyond the transition phase, I hold a deliberately minimal set of direct reports, each individually justified. The durable business runs through the two standing modeling charters under Alim and Daniel. I concentrate my own time on Reflex, with a direct pod scoped to serve exactly three things: the Reflex frontier, the UPP framework the org builds on, and Foundations & Efficiency. I keep Generative Retrieval as a time boxed incubation with explicit sunset criteria; as it graduates, my direct footprint shrinks further. The end state is a lean span I hold for clear reasons, not a wide span held indefinitely.

## Key Personnel Considerations

**Alim Virani (he/him), incoming EM, Anticipation Modeling.** Alim is motivated by growth into senior leadership and by visible team delivery. His management instinct, building teams that do not depend on him, matches the strong delegation model this charter needs, and his people leadership was the strongest in our interview loop. My coaching focus will be orienting his ambition toward organizational needs first. I am setting him up with a focused, deliverable rich charter from day one: both pUIC experiments land within his first month, with named senior support surrounding Chuxi's ramp. The initial pod differs modestly from what he heard during closing, primarily on senior anchoring; I will address that directly in his pre start call, and the Balaji question below bears on it.

**Daniel Liu, inherited EM, Retrieval Modeling.** Daniel is technical, straightforward, diplomatic, and delivers. He has been explicit that he wants his team on ML modeling work, and that is precisely his charter: frontier modeling that drives metric gains and publishes at venues like KDD and RecSys. He retains his boards surfaces, inherits LWS, and at the settle point sheds the retentive recs portion of his team to Alim, consolidating around one coherent modeling charter. Because he is onboarding an entire organization, I phase his scope deliberately; nothing is reshuffled while his team is landing. My first substantive conversation with him follows your announcement, and I frame every subsequent move as scope following people.

**Piyush Maheshwari, retrieval framework anchor (TL).** Piyush holds the deepest retrieval architecture context in the organization and today operates as the de facto senior TL across CLR, LWS, and UPP. The reorg could have fragmented that role; instead I resolve it structurally. I keep UPP with me as the organization's shared retrieval framework, and Piyush anchors it: he owns the foundation the modeling charters consume, a durable senior TL mandate rather than influence by adjacency. He is currently a single point of failure on UPP, which I am hedging through Zihao's ramp on cross surface training.

**Bella Huang, Staff, Generative Retrieval.** Bella is the Staff anchor for the Generative Retrieval (RecGPT) incubation and a continuity anchor through the transition. She is motivated by frontier generative work; the incubation gives her a clear mandate with explicit graduation criteria, and her scope grows as the bet matures.

**Balaji Rengarajan (he/him), Staff, Intelligent Boards.** Balaji is the senior Staff engineer on Intelligent Boards, joining with the Curation ML team. Two placements fit naturally in this design: platform TL within Retrieval Modeling, or Staff anchor for Anticipation Modeling. I will place him inside the observation window, after mapping the inherited workstreams with Daniel, and would value your read.

## Proposal

### Calls I've made

These are settled and do not depend on the observation window; the deferred moves below sequence off them.

1. **LWS → Daniel on day one.** He inherits its oncall and scoring-modeling work; any CLR consolidation is a settle-point call. This keeps blast radius low while his team lands and protects the Devin transition.
2. **Lionel → Alim's pod** (L14 SWE, Toronto, starts 7/27 with Alim). Serving has been the Retentive Recs bottleneck all year, and he is the only dedicated SWE in that space; starting the same day as Alim makes him a founding member, not the new engineer on an old team.
3. **Alok → Retentive Recs (primary, his own call) + Reflex.** RR loses Yuke to the single RecGPT stream, so Alok is load bearing for that pod alongside Chuxi and Yidi.
4. **JJ splits ~half Reflex / ~half Foundations & Efficiency** (responsiveness, L1 utility, cost — kept per your guidance against divesting small things), with **Rui as the L1 / Real-Time operational owner** underneath.
5. **CLR is not in Daniel's day-one scope.** Adding CLR on top of LWS and boards would overload his onboarding, so CLR stays with me transitionally (Devin leading) and targets Alim at the settle point, given the CLR↔Retentive synergy. **UPP stays with me** as the framework both CLR and LWS build on.

### Ownership

I assign accountability along four axes:

1. **Metrics.** Each leg carries named business metrics with a single threaded owner: Alim on retention and fresh content discovery, Daniel on modeling gains across boards and LWS preranking, my direct pod on Reflex adoption and incubation milestones. No metric has two owners.
...
3. **Prioritized support for other Pinterest workstreams.** A front door model gives leadership, partner organizations, and PMs a named owner per domain, so cross team asks arrive with context and receive a consistent response. This is the structural resolution of the routing gap Cupcake exposed. It is a routing map for asks originating outside the organization, not a gate on our own pods, which own their Pinner problems directly.
...
- **Generative Retrieval** graduates to Anticipation Modeling once incubation criteria are met — it is the proven gains engine that balances that team's frontier-heavy portfolio (RR, pUIC, exploration).
- **Intelligent Boards** stays with Daniel initially; I will read the synergy over the next two months. The deciding signal: whether IB gains originate in modeling improvements (it stays in Retrieval Modeling) or in surface pairings — notifications, Explore (it moves to Anticipation Modeling at settle). Dylan's notification-collaboration result suggests the latter, but the read is open.
- **Boards modeling depth**: how far Intelligent Boards and Recommend a Board grow as a frontier surface under Daniel, and whether boards modeling and LWS preranking share a pod. Gate: the inherited scope learning above.
...
| LWS | Daniel | Yali | Lightweight preranking modeling; oncall moves with the charter on day one |
...
1. **Balaji placement**: platform TL under Daniel, or Staff anchor under Alim. This intersects what Alim heard during closing, so your read matters. Default is to decide inside the observation window, after my first substantive conversation with Daniel.
