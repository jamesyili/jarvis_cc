...
3. **Cross-org initiatives.** Prioritized company efforts we support because we're accountable for the surfaces we own — even when it stretches us. In H2 2026: **NLFU Growth** (ISR: JJ · Modeling: Devin — the metric trade-off authorization expires end of September); **Content Quality** (JJ — Teen Safety, GenAI controls; scope and counterpart still thin); **See More / See Less** (Yali — a high-visibility co-ownership seat with Raymond Hsu on the front end, no primary or secondary); and **Content Exploration** (Bella — the newest, scope still firming up). Funded via named deliverables on existing engines, never ringfenced headcount.
...
- **LLM-pUIC appears on two maps.** It shows up in the Retentive Recs program (area 5) and in Daniel's Intelligent Board program (area 9). Designed overlap for now — Ling's pipeline serves both — but it's a named seam to settle, not one to discover.
...
| 6 | Reflex | Dafang | James |
| 7 | RecGPT / Generative Retrieval | Bella | James |
| 8 | Unified Explore Backend | Roderick | Daniel |
| 9 | LLM for Recommendations (Intelligent Boards, etc.) | Balaji | Daniel |
| 10 | Collection Personalization (Recommended Boards, etc.) | Yongwoo | Daniel |
...
- **Launch history (H1 2026):** the L1 layer matured into the org's control plane, and the responsiveness charter put its first big point on the board.
    ...
    - Responsiveness: [closeup cache invalidation for NLFUs](https://pinterest.slack.com/archives/C05UMECTDDJ/p1783447873989479) (Jul) — one of the largest NLF WAU wins to date (+37k WAUs, p=0.03), shipped on the legacy user-context sequence ahead of the real-time one; feeds the NLFU cross-org initiative (§2).
- **Risk:** JJ is the most-loaded name on this map — TL here, Build lead in Reflex, anchor of the infra campaign, and named on two cross-org initiatives (§2). That concentration is a design input for T2, not a footnote.
...
- **Politics:** exec attention on retention is real and personal — results here get read at levels above Dylan. Handle readouts with care: disclose early, frame honestly. And note the LLM-pUIC seam with area 9 (§2's confusion list) — designed overlap, named to be settled.
...
### 7. RecGPT / Generative Retrieval — TL: Bella
...
### 8. Unified Exploratory Backend — TL: Roderick

UEB is the bet that exploration should be a platform capability, not a new backend every time we have a product idea. It separates what interest to explore — UIC, heuristic or model-based pUIC, eventually LLM terms — from how we serve it: lifecycle, nomination, filtering, GULP execution, logging, and optional materialization. Recsplanation and Intelligent Boards already share most of these primitives; today they still pay the cost of partially separate stacks. The end state is simple: a new exploration approach should mostly be a config and policy change, not another orchestration fork. GULP remains the retrieval/ranking engine, unity-homefeed owns orchestration, and API/ngapi owns landing adaptation.

Where we are: Milestone 1 migrates the existing Exploratory Module with UIC first and pUIC immediately behind it. The gate is done; graph-builder scaffolding, the GULP mode, API Intelligent Boards refactor, and cross-feed impression dedup are in progress. Source selection, nomination, filtering, observability, E2E tests, and experiment wiring remain. The tracker points to core build Aug 21, bug bash Aug 28, initial ramp Aug 31, full experiment ramp Sep 8, and LR Sep 24. I do not want us to call this a platform win on parity alone. The first real proof is swapping UIC for pUIC without forking everything downstream.

Staffing: Roderick (TL, unity-homefeed and overall architecture), Sarah Tao on API/ngapi and the shared GULP landing path, Esteban and Lionel on GULP source modes, with Chuxi on pUIC attribution/logging. Jonatan is the current partner on direct exploratory-module wiring; several Unity/Helium tasks still need a named owner. H2 delivery arc:
* Milestone 1 — migrate UIC frontier sampling with parity, attribution, and one-switch rollback.
* pUIC expansion — change the source through config while preserving the same downstream contracts. This is the platform test.
* Intelligent Boards — reuse the core with a stronger exploitation policy and optional materialization. This is the second use case that tells us whether the abstraction is real.
* Explore Page comes after those two paths work; not before. We should earn the abstraction before broadening it.

Risk: medium-high. The risk is integration, not the box diagram: parity, attribution, GULP request semantics, dedup, and rollback all cross service boundaries. The other failure mode is familiar platform behavior — declaring victory when the scaffolding lands, or generalizing ahead of the second customer. Work type: distributed serving architecture and platform contracts, with experiment infrastructure and a meaningful relevance-quality boundary at GULP. Strategic weight: the force multiplier under the exploration portfolio. If this works, Recsplanation, pUIC, Intelligent Boards, and future bets can move independently without multiplying stacks. Bench: Roderick has the clearest end-to-end platform-design seat on the team. The test of his leverage is whether the next owner can onboard a module without Roderick rebuilding the path for them.

### 9. LLM for Recommendations / Intelligent Boards — TL: Balaji

Intelligent Boards is the product bet that Pinterest should do more than predict a promising next interest — we should turn that signal into a destination worth returning to. The exploratory cluster asks, "did we get the jump right?" An Intelligent Board is our response when the answer appears to be yes: a finite, offline-generated board with a title, recsplanation, cohesive pin set, cover, notification, and save path. The LLM is not decorative copy around a ranker here. It helps decide what the collection is, curate it as a whole, and clear a quality bar closer to an expert-created board than another feed.

Where we are: we're at week 8 of a 10-week, employee-dogfood-only sprint. The direction has converged from "IB chooses what to explore" to "exploration supplies evidence; IB crafts the response." The MVP generates after exploratory engagement, targets at least 40 pins, and carries one canonical title and recsplanation across the module, landing page, notification, and saved board. Two generation cores are being compared behind one shared spine: decomposed LLM topic synthesis → multi-modal expansion → LLM curation, versus Pin CLR retrieval → one-shot LLM curation. Cohesion, breadth, safety, cost, and latency decide the winner. We have meaningful substrate from the pUIC pipeline, GULP integration, debug tooling, dashboards, and next-board evaluation. What we do not have yet is a quality-gated product loop — and with two weeks left, the job is to land a credible dogfood experience, not imply that we have validated the product.

Staffing: Balaji is TL across the program. Ling owns the pUIC/LLM generation pipeline and evaluation foundation; Kimberly, Tianhao, Roderick, and Sreesha cover ML, backend, and serving integration; Jiaqi and Yash are building the client implementations. Edward and Daniel are the EM partners; Emun is PM. The ownership split across triggering, generation, evaluation, persistence, notification, and clients needs to stay explicit because this is a multi-team build, not one model launch.

MVP arc:
* Trigger from real engagement and reject topics that are not genuinely novel.
* Compare the two generation approaches with a human-calibrated LLM-as-judge rubric, then make the call rather than carrying both.
* Gate quality, safety, freshness, and duplication before materialization.
* Ship a consistent module/tab, notification, landing, save, fatigue, and replacement lifecycle.

Risk: high. Making one impressive board is easy relative to making millions of consistently cohesive, personal, safe boards. The current solution is expensive and difficult to scale, and its triggering path reaches a small user population; even a good experience may not have enough coverage to move topline metrics meaningfully. The 24–48 hour path makes trigger quality and re-entry load-bearing. Evaluation is the immediate risk: without a calibrated golden set and critic, we will optimize the demo and call it quality. Work type: LLM application and evaluation, retrieval and curation modeling, offline data pipelines, plus a full product lifecycle across backend and clients. Strategic weight: the flagship product expression of Anticipation and our clearest test of whether LLMs can create a new recommendation object, not just improve a ranking stage. If it works, it gives Pinterest a native answer to AI-generated personalization. Bench: Ling has the strongest emerging end-to-end LLM systems lane, from generic pUIC data and inference through the IB use case. Balaji's job is to make the quality bar and architecture call explicit enough that the capability outlives the sprint.

### 10. Collection Personalization: Recommended Boards & Board Ranker — TL: Yongwoo

Collection Personalization has two complementary engines. Recommended Boards decides which existing collections to show and makes the destination worth the tap; Board Ranker powers the board picker, predicting which board a Pinner wants to save to when they repin. One helps people discover the right collection, the other helps them build the right one. This is one curation loop: find a board, engage with it, and make the next save easier and more personal.

Recommended Boards has two branches. UGC Boards is the scaled personalization engine across P2P, Homefeed, and Notifications. It has already driven SSv2 and WAU, and I expect WAU to remain the largest forward opportunity as we improve quality and extend coverage. Featured Boards supports the editorial team's curated inventory. The impressions are high even though the corpus is small and engagement is low — however, the product generates tens of millions of $$ annually, so reliable ranking and editorial leverage matter.

Where we are: Recommended Boards is already a topline engine, not just a module-engagement story. The H1 launches improved the full path — user understanding, candidate generation, board ranking, and landing quality — and the notification expansion delivered a statistically significant WAU win. Board Ranker has also proven that better save-destination ranking can significantly drive sitewide and Homefeed repins, although it has not moved SSv2 yet. Its V3 rebuild fixes the data foundation, keeps all scored candidates and no-repin requests, and adds explicit board-source labels; training, offline evaluation, replay validation, and the next online experiment are running on top of that. The opportunity now is to make these gains compound: discovery creates better board engagement, better curation creates stronger future signals, and those signals improve the next recommendation.

Staffing: Yongwoo is TL across the workstream, with Ling leading Board Ranker modeling and the V3 training/evaluation line, Felix leading Recommended Boards candidate generation and experiments, and Josh and Tianhao from Edward's team owning the client implementation. Roderick is the team's overall backend lead, setting the architecture and technical direction across the portfolio. Yongwoo holds end-to-end technical accountability across ML, backend, and client. Launch history — UGC Recommended Boards (H1 2026):
* Pin2Board user-sequence ranking (Jan) — a transformer over recent pins plus shoppability features: SSv2 +0.10%, repins +0.35%, Other Board sessions +2.64%, outbound clicks +4.20%. The first clean proof that better user understanding in Recommended Boards moves more than the module.
* GULP landing-page reranking (Apr) — Other Board repins +10.9%, pin clicks +8.98%, time spent +6.89%, landing-page repin rate +20.38%, overall repins +0.23%. The lesson is straightforward: our job does not stop at the module tap.
* Manas candidate generation (Jun) — embedding ANN beside XPixie: Other Board sessions +4.03%, board views +3.61%, pin clicks +3.81%, outbound clicks +3.95%, neutral topline. It also found the next bottleneck. Coverage grew 13.6%, but thinner, less-diverse boards drove a roughly 13% landing-page repin-rate deficit and flat absolute repins. More candidates are not automatically better candidates.
* DCNv2 Pin2Board (Jun) — offsite user embeddings, recent-pin similarity, feature cleanup, and DCNv2: Other Board sessions +3.99%, repins +3.70%, pin clicks +4.73%.
* Realtime-action Board Recommendations notification (Jun) — global WAU +0.058% (~+260k), MAU +0.051% (~+354k), SSv2 propensity +0.05%, and email clicks +1.71%. This is the clearest proof that Recommended Boards can drive retention, and the strongest reason to keep pushing WAU.

Risk: moderate. Both engines have proven meaningful impact, but at different depths of the funnel. Recommended Boards already moves SSv2 and WAU; the challenge is sustaining and extending the WAU gains as coverage grows, without losing recommendation or post-tap quality. Board Ranker significantly moves sitewide and Homefeed repins but has not moved SSv2, so the open question is whether better save decisions create enough downstream session value to become a topline lever. The risk is not whether Collection Personalization can produce wins. It is whether those wins keep compounding through the full curation loop rather than plateauing at the action each model directly optimizes. Work type: full-fledged recommendation systems — retrieval and CTR modeling, product machine learning. Strategic weight: a proven SSv2 and WAU engine, a product generating tens of millions of $$ annually, and the ranking system behind one of Pinterest's core curation actions. This is also the collection substrate Intelligent Boards should reuse: board representations, quality signals, ranking infrastructure, and evaluation practice. Bench: Ling has a clear end-to-end Board Ranker lane; Felix has the candidate-generation and production-debugging lane. Roderick provides architectural coherence across the charter. Yongwoo's leverage is connecting them into one compounding collection system — recommendations improving curation, and curation improving the next recommendation.
