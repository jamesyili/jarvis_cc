...
Source: James supplied a photo of a Slack thread in #p13n-all; date is capture date (messages show relative timestamps). Partial transcription of readable substance; right-edge text in earlier messages is clipped. No image copied into the repo.

- Emun Solomon reports HF/Explore pages drastically homogenize upon IB save. Questions include whether similar activity reinforces one interest cluster, effects on Homefeed, dynamic UIC experiment timing, and whether grouping bulk events with normalization would help. The thread reports Roderick Gao and Yutong Jin confirmed filtering IB save actions from UIC generation for other products; this is reported thread context, not independently verified pipeline behavior.
- James's visible replies: bulk imports being passed as many repins dilute intentionality/specificity; suggested a separate event before A/B, citing prior NUX experience. Twenty repins could affect Homefeed through both UIC and user-sequence signals used by ranking/retrieval. Agreed with generalizing the solution and asked about PRD/engineering POC and sprint/results/A/B timing.
- Anna Kiyantseva asks that the solution generalize to similar non-pinner-initiated bulk-save cases, including meal-planner board creation via an assistant. Michael Weissinger reports personally experiencing the issue and agrees with James and Anna.
- Andrew Yaroshevsky: dogfooding IB made Pinterest feel new, fast and responsive, with new reasons to open it. A saved IB reaching the top nav and changing the Homefeed immediately is valuable; without a strong signal, integration/calibration could take quarters. Acknowledges his reaction may not represent pinners, agrees an experiment is needed, and wants the existing behavior as the dogfood baseline. Proposes retaining the action and adding metadata identifying a bulk save so the stack can treat it differently from a regular repin.

**James's follow-up position (same conversation):** (1) Calibration/tuning is necessary whichever event representation is chosen because user sequences are too strong a signal to leave untreated; he disputes that retaining repins saves that work. Differentiating the actions is the essential requirement. (2) Users cannot anticipate this magnitude of Homefeed change from the save action, and he says there is no way to reverse it once discovered. He raises confused users abandoning the app as a possible outcome, not an observed result. James asked Leo to evaluate these objections deeply; these are his arguments, not a ratified experiment or implementation decision.

**Evidence qualification (Leo, 9/8):** Pinterest's public [recommendation-control help](https://help.pinterest.com/en/article/tune-your-home-feed) documents turning off recommendations from specific boards. Whether that control is available/effective for this IB flow and reverses its user-sequence effects is unverified. Preserve James's concern about understandable, effective recovery; do not turn it into a confirmed claim that Pinterest has no recommendation controls.

No resolution or experiment decision is visible. James asked Leo to help think through a response; no reply has been sent by Leo.
