...
- **2026-08-19 — Dylan's H2 coaching mandate (H1-review 1:1; she named Alim twice):** (1) coach Alim (or Daniel) into *"someone who can be in the room when I am not — who can scale my judgment"* (today only Piyush clears that bar, per her); (2) on Anticipation: important mission, **no scope creep — create focus, limit the bandwidth spent**, coach Alim to focus on technology and **represent the team**. This is where she wants James's H2 coaching capital. Full entry: `dylan_wang_archive.md` 8/19.
- **2026-08-19 — wants RecGPT (second time), + his own ≤12-reports cap.** Inquired about RecGPT again in the 8/19 1:1 — James's read: "really passionate about that area." Notable timing: this is the same 1:1 window where the 30/60/90 (expansion = CLR per the 8/16 resolution) was to be shown — the passion survived seeing the CLR plan. Also stated he doesn't want more than 12 reports (Daniel independently said the same). Both feed the Fri 8/21 trio: the CLR-vs-RecGPT expansion call is flagged for James to reaffirm or re-open first (§T2 scenario boards, Update 8/19); Dylan's no-scope-creep mandate is the cover for time-gating whatever he gets.
...
- **2026-06-14 update — offer in hand + ER ticket filed.** Bella's external offer is now **confirmed/in hand** (past team-matching). James filed an **ER (Employee Relations) ticket** documenting the performance decline (email cites "noticeable decline over the past two months vs. her level — IC16 MLE — feedback given, no improvement"). **ER-ticket tactic — NOT Dylan-directed for Bella (corrected 2026-06-30):** the notion that an ER ticket makes an exit count as **managing out a low performer** (vs. regretted attrition), protecting the backfill req, was a **general tactic Dylan mentioned to James previously** — Dylan does **not** know about Bella's specific situation and did not direct this. (Earlier notes miscoded this as Dylan-directed for Bella; she has no knowledge of the case.) **Integrity check (James's honest read):** "a mix of the two" — there's a genuine headcount motive AND she **hasn't really performed well even from the beginning**, so the low-performer characterization has a real basis. Clean enough *because the substance is true*. Open: confirm with HR that the documentation actually secures the req (the perf-track and backfill-track are different mechanics).
  ...
  - **2026-08-19 — she is now a T2 design variable:** Daniel's CLR-over-RecGPT preference is substantially avoidance of working with her (James co-signs the difficulty read: "really hard to manage and probably to work with"); James's standing card for the trio — **"wherever RecGPT lands, Bella stays with me"** (line + management, charter matrixes) — unblocks Daniel's hedging and de-risks RecGPT for Alim simultaneously. Full context: `daniel_liu_archive.md` 8/19 + §T2 scenario boards Update 8/19.
  ...
  - **2026-08-22 — H1 review conversation DELIVERED (landed well) + response arc + new-CG strategy flashpoint + oncall unmoved + China emergency + 🔒 the Dafang disclosure.** (1) **The written H1 review conversation is delivered** — James: a "perfect conversation" (Yuke's too, separate). The heaviest carried perf block of the month is closed. (2) **Response arc — she's trying, and James sees it:** she is visibly picking up on his unhappiness (his bluntness registering) and showing up **every week in 1:1s with RecGPT experiments** (incl. "Helium Mix" ⟨name as heard⟩). **Two potential launches:** (a) an **SSv2-moving** candidate from further sizer tuning; (b) the **first proving-out of RecGPT L0+L1** — an additional pre-ranker inserted after the retriever, promising gains. Simulate likewise: **UI built, simulation paths working, proactive updates.** (3) **Strategy flashpoint:** her proposed next step for Hanlin = add RecGPT as a **new CG** rather than **replacing** the existing CG (the current experiment's design). James got unhappy and said exactly why — reads it as still fighting the as-few-models-as-possible consolidation direction. Her confusion, verbatim shape: *"I'm bringing you good results — why are you still unhappy?"* His reply: *"I don't understand why I have to strategize for you. Why can't you strategize? This is not the right strategy."* (Leo flag same-session: additive-CG is the standard risk-conservative rollout, so this is a strategy-direction disagreement, not strategy-absence. **RATIFIED + EXECUTED same day: James acknowledged the overreaction — "what she suggested was one strategy but there could be others" — and has ASKED Bella for a one-pager on the RecGPT strategy going forward**, one she and he can digest together. Grades against her H2 "proactive alignment-driving" criterion; watch for it landing before/around her 8/27 departure or after ~9/8 return.) (4) **Oncall: no movement from her.** Time-sensitive; James wants to start soon; worry = it redraws wounds with the EMs under Dhruvil and Yan, and some with Bella herself ⟨"draw some news on me" as heard — read as wounds/friction⟩. **RATIFIED 8/22 (same session): James runs the cross-EM alignment layer himself while she's out** — the wound-prone, James-altitude part anyway — and hands Bella execution when she's back ~9/8; no ask lands on her during the emergency. Prep item added to Notion (This Week) with a hard "nothing on Bella before 8/27" sub-item. (5) **China Thu 8/27, ~1.5 weeks (back ~9/7) — family emergency, NOT a trip.** James's posture: grace. (6) **🔒 SENSITIVE — James-only knowledge; Dylan does NOT know: Bella is married to Dafang He (Reflex TL).** She told James directly; independently corroborated — Dafang canceled his 1:1s with James in the same window (consistent with a shared family emergency; James believes her). Implications on file: (a) **info hygiene** — the Reflex/Dafang channel and the Bella perf channel are one household; assume anything said in either can reach the other; never explain Dafang's cancellations to anyone via the marriage (the secret leaks through empathy); (b) **flight-risk coupling** — her top-lab exit trigger is now a two-body problem (cuts both ways: stickier to move, or a dual departure if either lands OpenAI/Anthropic); (c) the disclosure itself is a **trust deposit** that cuts against the adversarial worst-case reading of her fortified posture; (d) the Reflex scheduler delegation to Dafang (8/22 morning) likely idles through the emergency — expectation re-timed in `../projects/reflex/program_state.md`; (e) **disclosure-trigger policy (PROPOSED by Karen 8/22, UNRATIFIED — James to set the actual condition):** James discloses to Dylan the moment Bella's case moves toward PIP/downlevel/exit, or the moment a Reflex decision materially touches her scope — so the secret is a chosen policy with a written trigger, never a default defended later under worse conditions. Until ratified, the standing fact is: no trigger condition exists.
...
- **2026-08-22 — H1 review conversation DELIVERED + the gentleman's agreement (plan-status change).** (1) **The written H1 review conversation is delivered and landed clean** — James: a "perfect conversation"; much clearer mutual understanding than 7/27. Yuke understood all the feedback, asked clarifying questions, **no fight-back this time** (contrast the 7/27 open contest). (2) **Delivery signal is real:** he's showing a **positive metric-moving RecGPT experiment**, says he'll try his best — and James has *seen* it: responding online, delivering. Evidence-bar note: this is artifact-level (experiments, online responses), not managed-up narrative — the specific counter to the 7/22 "can't tell whether he's producing" concern. (3) **The gentleman's agreement (James's frame):** Yuke will not stay forever; while here he is professional, responsive, and performs at **IC15 meeting-expectations**. Both sides understand this. (4) **Leo flag — plan status:** this de facto supersedes the 7/06 "when, not if" PIP lock. Live plan = **détente-with-documentation**: he performs at meets while he looks; the PIP stays pre-armed and fires only if he drops below meets; James keeps the ledger + same-day recaps running and **still quietly completes the Aug–Sept peer validation** — it protects both directions (corroborates meets if the delivery is real; preserves the case if it stops). With meets + an eventual self-directed exit, the Exceeds-2025 record obstacle goes moot — it only ever mattered for a contested PIP.
...
- **2026-08-25 — 1:1 (Tue): three flags via the skip channel + James's first hard growth feedback to her.** (1) **Wants RecGPT** — named the people (Bella, Yuke), not the work; James told her both are likely short-timers (Yuke distracted/looking, Bella maybe), she agreed. (2) **Burnout** — needs more vacation; James filed it and told her to take it. (3) **Doesn't enjoy working with the CLR side (Devin, Hongtao, Piyush)** — "not welcoming," a bad experience ⟨incident not captured — ask⟩; and says she **doesn't believe in conditional retrieval** ("the industry doesn't learn the conditions separate from the model") — near-verbatim Bella's 6/30 "stack cannot satisfy" line. **James's feedback, delivered:** talented performer, honest feedback for growth — work better with colleagues; if every friction lands on you like this you become hard to work with ("you need to grow up"). Cut short (James had to run); no close. **James's stance (8/26 AM):** no reorg change — she stays RR/pUIC under Alim; delivering RR + pUIC is non-negotiable (Andrew 8/25: pUIC = "the main bet of this whole thing"); maybe a heads-up; play it week by week. *Leo's read (8/26):* all three items are about people; the technical disbelief reads as **downstream of the CLR friction** (her ordering) and likely **borrowed from Bella**, not independent — less alarming than Bella's version, but Bella's frame is propagating. **Second collaboration gone sour in a month** (Alok 8/10→8/17 fight, now CLR) — the pattern James named is real and sits on the triangle's load-bearing seam (she was the bridge so Yuke/Devin never had to collaborate; Yuke's gone, the bridge now dislikes the far bank). **Follow-ups:** (a) next 1:1 — close the loop on "grow up" (one sentence: blunt, meant as the bet-on-you version) and give her the **positive half she hasn't heard**: RR/pUIC is the main bet; RecGPT is the pod Daniel avoided with a TL who has a named leave trigger. (b) Get the CLR incident before any heads-up. (c) Heads-up to Alim, if any, as **TL-staffing input about managing the CLR interface** — not "Chuxi is unhappy" (she carries the 8/10 Alim prior).
- **2026-08-26 AM — the CLR friction has a history (James), the 7:47 PM follow-up sent, and the plan.** **Origin:** when she joined she worked on **PinSpark** — a condition inside the CLR model — with heavy back-and-forth with **Hongtao + Piyush**, who threw ad-hoc asks at her (clean up train data etc.) without discussion or explanation; the asks were reasonable, the manner wasn't. James's hindsight: he should have smoothed it over then, when things were calm; instead he moved her onto RecGPT-side work with Zihao/Bella/Yuke ⟨"Zahav… ChatGPT and WeChatX" as dictated⟩ and the feelings toward Devin/Hongtao/Piyush never resolved — that bitterness now colors how she sees CLR. James agrees the technical objection is premature / "out of nowhere." **The conversation's actual close:** James asked her to consider that **Devin genuinely wants to work with her** — she said she **respects Devin** — and to calm down and try a collaboration with him at some point rather than conclude. **James's 7:47 PM Slack (sent, edited, unanswered as of 8/26 AM):** thanks her for her honesty; not easy to share her most direct thoughts; glad they've built mutual trust where each can open up; "this is also why I felt comfortable being so direct with you today." **Plan (Leo rec, 8/26; James asked book-now vs ping vs wait):** don't book or re-ping — the 7:47 message is complete and a second touch reads as checking on her; nothing in the conversation points at flight (she asked for vacation, agreed Yuke/Bella are leaving, accepted the Devin ask). If no reply by Thu midday → one content share, not a check-in (Andrew's "pUIC is the main bet"). **Mon 1:1 = the real close** (loop on "grow up" + RR-is-the-main-bet/RecGPT-reality framing). **James's move this week is on Devin's side of the bridge:** route her CLR interface through Devin (not Piyush/Hongtao); tell Devin she's open and respects him, and that the first ask must be well-scoped and explained — the opposite of the PinSpark pattern.
...
- **8/25 watch:** Yuke is off RR; the bridge itself is now strained — Chuxi says the CLR side (Devin, Hongtao, Piyush) is unwelcoming and she doesn't believe in conditional retrieval (details in her entry). The Chuxi↔Devin interface needs active management under Alim.
