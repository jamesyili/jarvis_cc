...
> **Source:** https://aman.ai/primers/ai/agentic-RL/ — offline text copy of the aman.ai primer (companion to `interview_prep/aman_agentic_rl.pdf`, which is image-only and not text-extractable).
...
* [Background](#background)
  + [When SFT Fails (and Why RL Is Required) for Tool-Calling Agents](#when-sft-fails-and-why-rl-is-required-for-tool-calling-agents)
  ...
  + [Teaching Agents Tool-Calling with RL](#teaching-agents-tool-calling-with-rl)
...
* [Reinforcement Learning for Web and Computer-Use Agents](#reinforcement-learning-for-web-and-computer-use-agents)
  ...
  + [pass@\(k\) vs. pass^\(k\): Measuring Capability vs. Reliability in Computer-Use Agents](#passk-vs-passk-measuring-capability-vs-reliability-in-computer-use-agents)
    - [Investigating Reliability in Agentic Execution](#investigating-reliability-in-agentic-execution)
  + [Methodology](#methodology)
    - [Paired Reliability Statistics](#paired-reliability-statistics)
  + [Sources of Unreliability](#sources-of-unreliability)
    - [Stochasticity](#stochasticity)
    - [Instruction Ambiguity](#instruction-ambiguity)
    - [Planning Variability](#planning-variability)
  + [Capability vs. Reliability: pass@\(k\) vs pass^\(k\)](#capability-vs-reliability-passk-vs-passk)
...
### Teaching Agents Tool-Calling with RL
...
* Proximal Policy Optimization (PPO), introduced in [Proximal Policy Optimization Algorithms](https://arxiv.org/abs/1707.06347) by Schulman et al. (2017), optimizes a clipped surrogate objective that constrains policy updates to remain close to the previous policy. The most commonly used objective is the clipped surrogate:
  ...
  \big)
  \Big]\]
  + … where the importance sampling ratio is:\[r\_t(\theta) = \frac{\pi\_\theta(a\_t \mid s\_t)}{\pi\_{\theta\_{\rm old}}(a\_t \mid s\_t)}\]
...
* More recent advancements such as **[OpenWebVoyager](https://arxiv.org/abs/2410.19609)** by He et al. (2024) brought these ideas into the multimodal realm. OpenWebVoyager extends open-source multimodal models (Idefics2-8B-Instruct) to perform real-world web navigation using both **textual accessibility trees** and **visual screenshots**. The training process unfolds in two phases:
  ...
  ![](/primers/ai/assets/RL-for-agents/OpenWebVoyager.jpg)
  ...
  ![](/primers/ai/assets/RL-for-agents/OpenWebVoyager2.jpg)
* Alongside real-environment exploration, a complementary approach is to scale policy learning with synthetic but reasoning-grounded interaction data. **DreamGym**, proposed in ([Scaling Agent Learning via Experience Synthesis](https://arxiv.org/abs/2511.03773) by Chen et al. (2025)), formalizes this by training a reasoning-based *experience model* that serves as both a generative teacher and an adaptive simulator. This model produces synthetic task curricula and consistent next-state transitions, enabling closed-loop reinforcement learning at scale.
  ...
  ![](/primers/ai/assets/RL-for-agents/DreamGym1.jpg)
* **Early Experience**, proposed in ([Agent Learning via Early Experience](https://arxiv.org/abs/2510.08558) by Zhang et al. (2025)), establishes a two-stage curriculum—implicit world modeling and self-reflection over alternative actions—that uses only language-native supervision extracted from the agent’s own exploratory branches, before any reward modeling or PPO/GRPO.
  ...
  ![](/primers/ai/assets/RL-for-agents/EarlyExperience1.jpg)
...
* One widely used algorithm is Proximal Policy Optimization (PPO) by [Schulman et al. (2017)](https://arxiv.org/abs/1707.06347), which introduces a clipped surrogate objective to ensure stable updates and avoid large shifts in policy space. [Policy/Preference Optimization](../preference-optimization) primer offers a detailed discourse of common policy/preference optimization algorithms in the context of LLMs.
...
* [Reinforcement Learning Enhanced LLMs: A Survey](https://arxiv.org/abs/2412.10400v1) offers an overview of this field.
...
* A typical pipeline to train a web or computer-use agent might follow:
  ...
  6. Run RL policy optimization in the environment: sample trajectories, estimate advantages/returns, update policy using PPO or variants.
...
### pass@\(k\) vs. pass^\(k\): Measuring Capability vs. Reliability in Computer-Use Agents

* [On the Reliability of Computer Use Agents](https://arxiv.org/abs/2604.17849) by Gonzalez-Pumariega et al. (2026) reframes evaluation for computer-use agents around reliability rather than one-off capability. The core problem is that an agent can solve a task once yet fail on repeated executions of the same unchanged task, which means single-run success and best-of-\(k\) success can overstate deployability.
...
* This reflects a move from optimizing expected success toward optimizing reproducible success. More broadly, the field appears to be moving from pass@\(k\) toward pass^\(k\), from evaluating what agents can do to evaluating what can be deployed. In that sense, pass^\(k\) is not merely an evaluation metric but is increasingly emerging as a training objective centered around reproducibility, robustness, strategy stability, and repeated-run success guarantees.
* The central distinction is between pass@\(k\) and pass^\(k\). pass@\(k\) measures capability, whether at least one of \(k\) attempts succeeds, while pass^\(k\) measures reliability, whether repeated executions succeed consistently.
* The following figure ([source](https://arxiv.org/abs/2604.17849)) shows shows Agent S3 with GPT-5 reaches roughly 78% pass@10, while pass^10 is only about 36%, demonstrating that “can solve once” and “can solve every time” are fundamentally different operational properties. Specifically the figures indicates: (left) Performance of a strong computer-use agent (Agent S3 with GPT-5) across repeated attempts. While Pass@10 reaches approximately 78%, the corresponding Pass^10 indicates that the agent succeeds on all 10 executions for only about 36% of tasks, indicating that achieving reliability across repeated executions is challenging. (right) Overview of factors that contribute to unreliability. [On the Reliability of Computer Use Agents](https://arxiv.org/abs/2604.17849) by Gonzalez-Pumariega et al. (2026) decomposes task execution into three components: stochasticity during execution, ambiguity in task specification, and variability in agent behavior, which adversely affect reliability across repeated executions.

![](/primers/ai/assets/RL-for-agents/perf-CUA.jpg)

#### Investigating Reliability in Agentic Execution

* [On the Reliability of Computer Use Agents](https://arxiv.org/abs/2604.17849) by Gonzalez-Pumariega et al. (2026) asks whether solving a task once implies the agent can solve it reliably. Their answer is no, because reliability failures arise from three structural sources: stochasticity during execution, ambiguity in task specification, and variability in agent behavior. These are not incidental noise sources, but persistent obstacles to dependable computer-use policies.
* The paper formalizes computer-use agents as a partially observable Markov decision process:
  ...
  R
  \rangle\]
  + where \(\mathcal S\) is the computer state space, \(\mathcal O\) the observation space, \(\mathcal A\) the action space, \(\mathcal T\) the stochastic transition function, \(\mathcal I\) the instruction space, and \(R\) the reward function over trajectories and instructions.
* A task is defined as:

  \[x := (s\_0, I) \in \mathcal X\]
  + with trajectory:

    \[\tau
    =(s\_0,a\_0,\ldots,a\_{T-1},s\_T)\]
  + and standard evaluation often focuses only on:

    \[\mathbb E\_{\tau \sim \pi(\cdot|x)}
    [R(\tau,x)]\]
...
\[J\_{\text{reliable}}(\pi)
=J\_{\text{task}}(\pi)
-\lambda\_1 U\_{\text{stoch}}
-\lambda\_2 U\_{\text{amb}}
-\lambda\_3 U\_{\text{plan}}\]
...
* For each task \(x\), the policy is executed \(n\) times, yielding binary outcomes:
  ...
  + with success count:
    ...
    =\sum\_{j=1}^{n}
    r\_{x,j}\]
* Tasks are then categorized as consistently solved when \(c\_x=n\), inconsistently solved when \(0<c\_x<n\), and never solved when \(c\_x=0\). This repeated-run framing is the basis for reliability analysis.
...
* The following figure ([source](https://arxiv.org/abs/2604.17849)) illustrates three metrics for analyzing consistency in agent performance over multiple runs of the same task. (a) pass^\(k\) (repeated-run success) estimates the probability that \(k\) executions of a task succeed, averaged across tasks. (b) McNemar measures improvements and regressions in reliability between two settings by counting tasks that transition between being consistently solved and not. (c) Wilcoxon signed-rank test compares per-task success counts across settings, capturing incremental changes in consistency even when full reliability is not achieved.

![](/primers/ai/assets/RL-for-agents/consistency-metrics.jpg)

#### Paired Reliability Statistics

* To study task-level reliability shifts, [On the Reliability of Computer Use Agents](https://arxiv.org/abs/2604.17849) by Gonzalez-Pumariega et al. (2026) complements aggregate pass^k metrics with paired statistical tests that analyze whether an intervention improves reliability task by task, rather than only in expectation.
* The paper first uses McNemar’s test to measure whether more tasks become reliably solved than regress. For each task, define a binary reliability indicator:
  ...
  \[z\_x
  =\mathbf 1[c\_x=n]\]
  + where \(z\_x = 1\) means task \(x\) succeeds on all repeated runs and is therefore reliably solved.
...
* The statistic is:
  ...
  =\sum\_{d\_x>0}
  R\_x\]
  + where \(R\_x\) is the rank of \(\| d\_x \|\) among nonzero differences.
* Conceptually:
...
* Multiple reasonable interpretations may produce inconsistent outcomes despite competent behavior. Clarifying instructions significantly improves pass^\(k\), showing unreliability can arise from latent reward uncertainty rather than policy weakness.
...
* This suggests clarification is itself an RL action. A reliability-aware policy may optimize:
  ...
  \[\pi(a,q|s)\]
  + where \(q\) includes clarification choices, linking computer-use agents to active querying and preference-based RL.
...
### Capability vs. Reliability: pass@\(k\) vs pass^\(k\)

* This paper sharpens a foundational distinction. pass@\(k\) rewards finding one successful strategy:

  \[\exists \tau
  \text{ successful}\]
  + whereas pass^\(k\) rewards repeatedly selecting stable strategies:
...
* [On the Reliability of Computer Use Agents](https://arxiv.org/abs/2604.17849) by Gonzalez-Pumariega et al. (2026) shows removing stochasticity alone is not enough, deterministic decoding does not guarantee reliability, instruction ambiguity is a major hidden source of unreliability, and clarification can outperform repeated retries. Planning variability can itself be a failure mode, implying reliable agents require robust strategies, not just successful ones.
