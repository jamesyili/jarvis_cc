...
> **Source:** https://aman.ai/primers/ai/mixture-of-experts/ — offline text copy of the aman.ai primer (companion to `interview_prep/aman_moe.pdf`, which is image-only and not text-extractable).
...
* [Ultra-Sparse Frontier MoEs: Kimi K2, Sparsity Scaling, and System Co-Design](#ultra-sparse-frontier-moes-kimi-k2-sparsity-scaling-and-system-co-design)
  + [Why Kimi K2 Matters for MoE Scaling](#why-kimi-k2-matters-for-moe-scaling)
  + [Sparsity as a Scaling Variable](#sparsity-as-a-scaling-variable)
  + [Kimi K2’s Sparsity Scaling Law](#kimi-k2s-sparsity-scaling-law)
  + [Attention Design and Long-Context Cost](#attention-design-and-long-context-cost)
  + [Routing Stability and Load Balance](#routing-stability-and-load-balance)
  + [Activation-Scale Stability](#activation-scale-stability)
  + [Optimizer Stability with MuonClip](#optimizer-stability-with-muonclip)
  + [Expert-Parallel Communication and Training Infrastructure](#expert-parallel-communication-and-training-infrastructure)
...
![](/primers/ai/assets/mixture-of-experts/MoE_overview.jpeg)
...
* Modern models such as **Mixtral-8×7B**, **DeepSeek-V2**, **Gemini 1.5**, and **Claude 3** continue this lineage by embedding MoE principles not only in feed-forward layers but also in attention and cross-modal fusion mechanisms. Specifics below:
...
* A recent frontier-scale example is [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025), which advances sparse MoE design by scaling to 1.04T total parameters with 32B activated parameters, 384 experts, 8 activated experts per token, and sparsity 48; its design shows that modern MoE scaling is increasingly governed by empirical sparsity laws, routing stability, optimizer stability, and expert-parallel communication efficiency rather than parameter count alone.
...
![](/primers/ai/assets/mixture-of-experts/hoe.gif)
...
![](/primers/ai/assets/mixture-of-experts/moe.jpg)
...
## Ultra-Sparse Frontier MoEs: Kimi K2, Sparsity Scaling, and System Co-Design

### Why Kimi K2 Matters for MoE Scaling

* [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) is a frontier-scale example of hardware-aware sparse scaling, presenting a 1.04T-parameter MoE Transformer with approximately 32B activated parameters, 384 total experts, 8 activated experts per token, and a sparsity level of 48.
* This design extends the MoE trajectory beyond earlier sparse systems such as [GShard](https://arxiv.org/abs/2006.16668) by Lepikhin et al. (2020), [Switch Transformers](https://arxiv.org/abs/2101.03961) by Fedus et al. (2021), [Mixtral of Experts](https://arxiv.org/abs/2401.04088) by Jiang et al. (2024), and [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437) by DeepSeek-AI et al. (2024), moving from sparse activation as a fixed architectural recipe toward sparsity as an empirically tuned scaling variable.
* More broadly, Kimi K2 shows that sparse architectures are only efficient when token dispatch, expert computation, communication overlap, memory layout, activation management, and optimizer stability are co-designed. This continues the system-level MoE trajectory established by [GShard](https://arxiv.org/abs/2006.16668) by Lepikhin et al. (2020), [Dropless MoE](https://arxiv.org/abs/2211.15841) by Gale et al. (2022), [DeepSpeed-MoE](https://arxiv.org/abs/2201.05596) by Rajbhandari et al. (2022), and [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437) by DeepSeek-AI et al. (2024).
* Kimi K2 is not merely a larger MoE; it is a more sparse, more expert-heavy, and more inference-conscious MoE. Relative to DeepSeek-V3, it keeps the layer count fixed at 61, increases total parameters from 671B to 1.04T, reduces activated parameters from 37B to 32.6B, increases routed experts from 256 to 384, keeps 8 active experts per token, keeps 1 shared expert, reduces attention heads from 128 to 64, reduces dense layers from 3 to 1, and removes expert grouping.
* The main design lesson is that the frontier question is no longer simply whether sparse expert activation can scale, but how far sparsity can be increased before routing imbalance, activation-scale drift, attention instability, memory pressure, and all-to-all communication erase the compute advantage. In this sense, sparsity 48 represents a practical engineering knee in the quality–cost curve rather than a universal optimum.

### Sparsity as a Scaling Variable

* In a top-\(k\) MoE layer, sparsity can be defined as the ratio between the total number of routed experts and the number of experts activated per token:

  \[\text{sparsity} = \frac{E\_{\text{total}}}{E\_{\text{active}}}\]
  + where \(E\_{\text{total}}\) denotes the number of routed experts and \(E\_{\text{active}}\) denotes the number of selected experts per token.
* Under this definition, [Mixtral of Experts](https://arxiv.org/abs/2401.04088) by Jiang et al. (2024) has sparsity \(\frac{8}{2} = 4\), [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437) by DeepSeek-AI et al. (2024) uses 256 routed experts with 8 active experts per token for sparsity \(\frac{256}{8} = 32\), and [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) increases this to \(\frac{384}{8} = 48\) while keeping the number of active experts fixed.
* The central benefit of higher sparsity is that total model capacity can increase while per-token expert compute remains approximately tied to \(E\_{\text{active}}\) rather than \(E\_{\text{total}}\):

  \[C\_{\text{token}} \propto E\_{\text{active}}
  \quad \text{while} \quad
  P\_{\text{total}} \propto E\_{\text{total}}\]
  + where \(C\_{\text{token}}\) is the approximate per-token expert compute and \(P\_{\text{total}}\) is the total expert-parameter capacity.
* However, ultra-high sparsity is not automatically beneficial. Increasing expert count can intensify routing imbalance, expert under-training, token overflow, and communication overhead, which is why [Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer](https://arxiv.org/abs/1701.06538) by Shazeer et al. (2017) introduced auxiliary load-balancing losses, [Switch Transformers](https://arxiv.org/abs/2101.03961) by Fedus et al. (2021) formalized capacity-aware routing constraints, and [ST-MoE: Designing Stable and Transferable Sparse Expert Models](https://arxiv.org/abs/2202.08906) by Zoph et al. (2022) studied stability and transfer issues in sparse expert models.

### Kimi K2’s Sparsity Scaling Law

* [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) reports sparsity-scaling experiments in which the number of activated experts is fixed at 8, the number of shared experts is fixed at 1, and the total number of experts is varied. Under this controlled setting, increasing sparsity consistently lowers training and validation loss at fixed activated parameters, meaning that greater total expert capacity improves loss-per-FLOP when the active computation budget is held constant.
* The following figure ([source](https://arxiv.org/abs/2507.20534)) shows the sparsity scaling law: increasing sparsity leads to improved model performance, with 8 activated experts and 1 shared expert held fixed while the total number of experts is varied.

![](/primers/ai/assets/mixture-of-experts/Kimi-K2.pdf_fig5_sparsity-scaling-law.jpg)

* At a validation loss of 1.5, [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) reports that sparsity 48 reduces FLOPs by 1.69×, 1.39×, and 1.15× relative to sparsity levels 8, 16, and 32, respectively.
* The plotted family also includes a sparsity-64 curve, but the adopted production configuration is sparsity 48, indicating that the final design is governed by the quality–cost frontier rather than by the largest tested sparsity alone.
* This result illustrates a diminishing-return regime. Moving from sparsity 8 to 16 gives a larger compute reduction than moving from sparsity 32 to 48, and the final choice is framed as a balance between model performance and infrastructure complexity. Thus, sparsity 48 should be understood as an engineering knee in the quality–cost curve rather than a universal optimum.

### Attention Design and Long-Context Cost

* Kimi K2’s sparsity decision is coupled to its attention design. [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) uses Multi-head Latent Attention (MLA), a model hidden dimension of 7168, and an MoE expert hidden dimension of 2048, while reducing the number of attention heads from 128 in DeepSeek-V3 to 64.
* This matters because long-context agentic workloads are sensitive to attention cost. [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) reports that increasing attention heads from 64 to 128 at sequence length 128k would raise inference FLOPs by 83% while producing only modest validation-loss gains of roughly 0.5% to 1.2% in controlled experiments.

### Routing Stability and Load Balance

* Ultra-sparse MoE designs expose a three-way trade-off:

  \[\text{quality gain} \;\leftrightarrow\; \text{routing stability} \;\leftrightarrow\; \text{communication cost}\]
  + Higher sparsity increases available expert capacity, but it also requires the router to distribute tokens across more experts and requires the system to move token activations across devices efficiently.
* One difficulty is routing imbalance: if expert \(i\) receives a token fraction \(f\_i\) much larger than the ideal \(\frac{1}{E\_{\text{total}}}\), it receives more gradient updates, learns faster, and may become even more likely to be selected.
* Classical MoEs handle routing imbalance with an auxiliary balancing term such as the Switch-style objective:

  \[\mathcal{L}\_{\text{balance}} = \lambda E \sum\_{i=1}^{E} f\_i P\_i\]
  + where \(f\_i\) is the realized fraction of tokens routed to expert \(i\), \(P\_i\) is the average router probability assigned to expert \(i\), \(E\) is the number of experts, and \(\lambda\) controls the strength of the auxiliary term.
* A limitation of auxiliary load-balancing losses is that they introduce an optimization objective that can compete with the language-modeling objective. [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437) by DeepSeek-AI et al. (2024) uses an auxiliary-loss-free load-balancing strategy based on dynamically adjusted expert biases, making it an important predecessor for modern high-sparsity MoE routing.

### Activation-Scale Stability

* Another issue is activation-scale stability. If only \(k\) experts contribute non-zero outputs, then naive averaging can reduce the expected output magnitude relative to the input scale. A simple normalization intuition is that the routed expert output should be rescaled by approximately:

  \[s \approx \sqrt{k}\]
  + where \(k\) is the number of active routed experts.
* For Kimi K2’s \(k = 8\) active routed experts, this gives:

  \[\sqrt{8} \approx 2.828\]
  + This provides a natural scale for stabilizing the combined expert output when only a small subset of experts is active per token.

### Optimizer Stability with MuonClip

* A further scaling issue is optimizer stability at trillion-parameter scale. [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) introduces MuonClip, which combines Muon-style token-efficient optimization with QK-Clip, a weight-clipping mechanism that constrains attention logits by rescaling query and key projection weights when per-head logits exceed a threshold \(\tau\).
* For each attention head \(h\), Kimi K2 defines the batch maximum attention logit as:

  \[S^{h}\_{\max}
  =
  \frac{1}{\sqrt{d}}
  \max\_{X \in \mathcal{B}}
  \max\_{i,j}
  Q\_i^h {K\_j^h}^{\top}\]
  + where \(Q^h = XW\_q^h\), \(K^h = XW\_k^h\), \(d\) is the head dimension, and \(\mathcal{B}\) is the current batch.
* QK-Clip then applies a per-head scaling factor:

  \[\gamma\_h = \min\left(1, \frac{\tau}{S^{h}\_{\max}}\right)\]
  + so that unstable heads are clipped without unnecessarily perturbing all attention heads.
* For MLA, [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) applies clipping only to unshared attention components: head-specific query and key components are scaled by \(\sqrt{\gamma\_h}\), the head-specific rotary query component is scaled by \(\gamma\_h\), and the shared rotary key component is left untouched. This makes QK-Clip compatible with MLA, where key matrices are not fully materialized during inference.
* The following figure ([source](https://arxiv.org/abs/2507.20534)) shows (left) vanilla Muon can drive attention logits above 1000 in a mid-scale MoE run, while Kimi K2 with MuonClip; (right) \(\tau = 100\) keeps maximum logits capped early in training before they decay to a stable range.

![](/primers/ai/assets/mixture-of-experts/Kimi-K2.pdf_fig2_muonclip-logits.jpg)

* Kimi K2’s stable pre-training curve is also a central part of the sparsity story: [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) reports pre-training on 15.5T curated tokens with no observable loss spikes, using a 4096-token context window, a WSD learning-rate schedule, a global batch size of 67M tokens, 10T tokens at a constant learning rate after warm-up, 5.5T tokens under cosine decay, and a later context-extension phase to 32k followed by YaRN-based extension to 128k.

### Expert-Parallel Communication and Training Infrastructure

* In expert-parallel MoE training, token representations must be dispatched from the devices holding the current tokens to the devices holding the selected experts, and then combined after expert computation. This cost can be summarized as:

  \[t\_{\text{MoE}} \approx t\_{\text{expert-compute}} + t\_{\text{dispatch}} + t\_{\text{combine}}\]
  + where \(t\_{\text{dispatch}}\) and \(t\_{\text{combine}}\) are communication terms that grow in importance as expert parallelism and sparsity increase.
* Kimi K2’s training system is designed around this communication constraint. [Kimi K2: Open Agentic Intelligence](https://arxiv.org/abs/2507.20534) by Kimi Team et al. (2025) uses a combination of 16-way Pipeline Parallelism with virtual stages, 16-way Expert Parallelism, and ZeRO-1 Data Parallelism.
* Under this setup, BF16 model parameters plus FP32 gradient accumulation buffers require approximately 6 TB of GPU memory across a 256-GPU model-parallel group, while the system keeps roughly 30 GB per GPU for model states and uses the remaining memory for activations.
* The choice of EP = 16 is especially important for sparsity 48. Since each EP group holds the distributed expert set, Kimi K2’s 384 experts imply approximately 24 experts per expert-parallel shard:

  \[\frac{384}{16} = 24\]
  + This keeps each shard large enough to host many experts but small enough to reduce the latency of expert-parallel communication.
* The infrastructure further overlaps expert-parallel all-to-all communication with computation under an interleaved 1F1B schedule. Weight-gradient computation is decoupled from each micro-batch backward pass and run in parallel with pipeline communication, while selective recomputation, FP8 storage for insensitive activations, and CPU activation offload reduce memory pressure.
...
![](/primers/ai/assets/mixture-of-experts/clustr.jpeg)
...
![](/primers/ai/assets/mixture-of-experts/impl.jpeg)
