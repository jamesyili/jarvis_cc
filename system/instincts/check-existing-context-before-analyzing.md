...
### 2026-08-01b — cross-system delta ingestion: diff claims against local records before propagating
The Work-Leo org-planner delta included "the L1/Real-Time pager follows the LWS charter to Daniel." Leo propagated it into the delta record + file_index without checking it against this repo's own oncall record — which says L1/Real-Time ops = Rui under F&E (the no-pager-gap design). James: "What the fuck is the L1/Real-Time pager — cut that out, that's AI garbage." The contradiction was checkable locally before propagation. Heuristic: when ingesting a delta authored by another system (work-leo, a tool, an agent), diff each load-bearing claim against the local record first — contradictions get FLAGGED to James, not propagated; the delta's own confidence is not evidence.
