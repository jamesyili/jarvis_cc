...
**Source:** https://louiswang524.github.io/blog/genrec-paradigm-comparison/
**Ingested:** 2026-04-05
**Tags:** llms, recsys, ai-agents, ml-systems

---

[Home](/) › [Blog](/blog) › Two Bets on Generative Recommendation: Semantic IDs vs. Fine-Tuned LLMs  
Two Bets on Generative Recommendation: Semantic IDs vs. Fine-Tuned LLMs
 
 March 29, 2026 · 17 min read  · [recommendation systems](/tags/recommendation systems)[generative AI](/tags/generative AI)[LLM](/tags/LLM)[semantic IDs](/tags/semantic IDs)[TIGER](/tags/TIGER)[HSTU](/tags/HSTU)[PLUM](/tags/PLUM) 
...
[Paradigm A: Semantic ID-Based Autoregressive Models](#paradigm-a-semantic-id-based-autoregressive-models)
...
The result is a short token sequence `[C₁, C₂, C₃, ...]` where the first token encodes broad category, and each subsequent token encodes finer detail. Two NBA highlight clips will share C₁ (sports) and C₂ (basketball) but differ at C₃ (specific game, specific player angle). Two items sharing a code prefix are semantically related — and the autoregressive model can exploit that structure to recommend related items it has never seen in training.
 ...
 RQ-VAE codebook: hierarchical item tokenization with prefix sharing   
 ...
 residual ε₁   VQ Round 2 

  C₂  

 residual ε₂   VQ Round 3 

  C₃  

   [ C₁ · C₂ · C₃ ]  Quantization pipeline  

  
Similar items share prefix
  Item A Item B    C₁  C₂   C₁  C₂    C₃ₐ  C₃ᵦ   
 ...
 differ  Semantic similarity via shared prefix 
...
[Training the sequence model](#training-the-sequence-model)
...
[Paradigm B: Fine-Tuned LLMs for Recommendation](#paradigm-b-fine-tuned-llms-for-recommendation)
...
**Preference alignment:** A small number of systems extend fine-tuning with preference data using techniques inspired by RLHF (Reinforcement Learning from Human Feedback). The model is trained on pairs of (recommended item, rejected item). This steers it toward items users genuinely engage with, not just items that appear next in training sequences.
 ...
 LLM prompt serialization: user history formatted as a text prompt for recommendation   

      [User History]  
"Introduction to Python"  ·  watched 3 days ago
 
"ML Crash Course"  ·  watched 2 days ago
 
"Deep Learning with PyTorch"  ·  watched 1 day ago
  

  [Task] 
Recommend the next item this user will watch:
   [GENERATE →]  

 autoregressive generation   
"Neural Networks for Beginners"  (video)
...
[Production systems](#production-systems-1)
...
[Head-to-Head: Trade-off Analysis](#head-to-head-trade-off-analysis)
...
The two paradigms have substantially different profiles across the dimensions that matter most in production. The radar chart below makes the contrast visual; the table and explanations below make it precise.
 ...
 Radar chart comparing semantic ID models and fine-tuned LLMs across six trade-off dimensions  
   ...
   Cold-start  Data Efficiency  Item Churn  Personalization  Speed  Cost Efficiency    Semantic ID models  Fine-tuned LLMs 
...
DimensionSemantic ID ModelsFine-tuned LLMs

**Cold-start**WeakStrong

**Data efficiency**Needs large interaction logsLeverages pretraining

**Item churn handling**Brittle — codebook rebuild requiredRobust — new items described in text

**Personalization depth**Strong — end-to-end behavioral trainingDepends on fine-tuning quality

**Serving latency**Fast — small vocab, trie decodingSlow — large model, long context

**Serving cost**LowerHigher — requires aggressive caching
...
[Personalization depth](#personalization-depth)
...
[Serving latency and cost](#serving-latency-and-cost)
...
[Convergence: The Two Paradigms Are Borrowing from Each Other](#convergence-the-two-paradigms-are-borrowing-from-each-other)
...
The contrast laid out above is real — but unstable. The most capable systems in production today don’t sit cleanly in one camp. They borrow the other’s best ideas.
 ...
 Convergence diagram: Semantic ID models and Fine-tuned LLMs borrowing techniques from each other   

   

     Semantic ID Models custom token vocab behavioral training fast inference weak cold-start   Fine-tuned LLMs natural language vocab world knowledge robust item churn strong cold-start  

 
PLUM adopts RQ-VAE codes →
  

 
← LUM injects LLM representations
  shared substrate: transformer decoder 
...
[LLM systems adopting semantic IDs](#llm-systems-adopting-semantic-ids)
...
[ID-based systems borrowing LLM representations](#id-based-systems-borrowing-llm-representations)
...
[The converging question](#the-converging-question)
...
[So What: When to Choose Which](#so-what-when-to-choose-which)
...
Zhang et al. (2024). [LlamaRec: Two-Stage Recommendation using Large Language Models for Ranking](https://arxiv.org/abs/2407.07855).
 ...
 [ ← Previous The Attention Bottleneck: How Modern LLMs Solved a Problem That Nearly Broke the Transformer ](/blog/attention-variants) 
 
 [ Next → Demystifying Claude Code: Inside the Architecture of a CLI Code Agent ](/blog/demystifying-claude-code-architecture)
